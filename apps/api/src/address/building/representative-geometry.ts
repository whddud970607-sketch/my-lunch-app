...
function ringSignedAreaAndCentroid(ring: Ring | null | undefined) {
  ...
  return {
    area: twiceArea / 2,
    centroid: [cx / (3 * twiceArea), cy / (3 * twiceArea)] as Coord,
  };
}

/** Orientation-independent polygon centroid with holes (GeoJSON ring order). */
export function polygonCentroidWithHoles(rings: PolygonCoords | null | undefined): Coord | null {
...
export function pointInRing(point: Coord, ring: Ring) {
  const [x, y] = point;
  let inside = false;
  for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
    const [xi, yi] = ring[i];
...
export function pointInPolygonWithHoles(point: Coord, rings: PolygonCoords) {
...
export function pointInMultiPolygon(point: Coord, multiCoords: MultiPolygonCoords) {
...
function bboxOfRings(rings: PolygonCoords) {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
...
function xIntersectionsAtY(ring: Ring, y: number) {
  const xs: number[] = [];
  for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
    const [x1, y1] = ring[j];
    const [x2, y2] = ring[i];
...
function interiorSegmentsAtY(rings: PolygonCoords, y: number) {
...
function polygonAbsArea(rings: PolygonCoords) {
 ...
 */
export function customInteriorPointPolygon(rings: PolygonCoords) {
  const centroid = polygonCentroidWithHoles(rings);
  if (centroid && pointInPolygonWithHoles(centroid, rings)) {
    return {
      point: centroid,
      method: "CUSTOM_INTERIOR_POINT" as const,
  ...
  return { point: null, method: "CUSTOM_INTERIOR_POINT" as const, strategy: "failed" };
}

export function multiPolygonCentroid(multiCoords: MultiPolygonCoords): Coord | null {
  let total = 0;
  let cx = 0;
  let cy = 0;
  for (const poly of multiCoords) {
...
export function customInteriorPointMultiPolygon(multiCoords: MultiPolygonCoords) {
  let bestPoly: PolygonCoords | null = null;
  let bestArea = -1;
  for (const poly of multiCoords) {
    const a = polygonAbsArea(poly);
    if (a > bestArea) {
...
export function computeRepresentativePoints(geometry: GeoJsonGeometry | null | undefined) {
  let multiCoords: MultiPolygonCoords | null = null;
  if (geometry?.type === "MultiPolygon") multiCoords = geometry.coordinates as MultiPolygonCoords;
  else if (geometry?.type === "Polygon") multiCoords = [geometry.coordinates as PolygonCoords];
  else return null;
